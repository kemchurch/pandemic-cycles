function DF = function_DF(SI,p,alpha,para,N)
[S,I]=convert_1to2(SI,N);
S = reshape(S,[N+1,4]);
I = reshape(I,[N+1,4]);
DF = zeros(2+4*2*(N+1),2+4*2*(N+1));
alt = 1-2*rem(1:N,2);

% Derivatives of BVPs wrt (S,I)_idx
for idx=0:3
    % n/=/0
    [DFS0,DFI0] = cheb_ODE_derivative_bczero(S(:,idx+1),I(:,idx+1),p,para,idx,N);
...
% Derivatives wrt p
DF(:,end-1:end) = cheb_ODE_derivative_bczero_para(S,I,para,N);
