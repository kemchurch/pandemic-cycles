function [DFS,DFI] = cheb_ODE_derivative_bczero(S,I,p,para,idx,N)
% Parameters
if idx==0 || idx==1
    beta = para(2);
elseif idx==2 || idx==3
...
nminusJ = abs(n-J)+1;
nplusJ = abs(n+J)-1;
DFS = [diag(2*(0:N)'),zeros(N+1,N+1)];
DFI = [zeros(N+1,N+1),diag(2*(0:N)')];
I_ext = [I;zeros(N+2,1)];
S_ext = [S;zeros(N+2,1)];
DFS_nL = zeros(N+2,2*(N+1));
DFI_nL = zeros(N+2,2*(N+1));
