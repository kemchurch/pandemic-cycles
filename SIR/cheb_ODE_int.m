function F = cheb_ODE_int(S,I,Splus,Iplus,p,para,idx,N,dflag)
% Notes:
...
T = -diag(n,-1) + diag(n,1);
T(1,:) = 0;
SI_conv = quadratic_cheb_intval([S;0],[I;0]);
F = dflag*repmat(2*(0:N)',[2,1]).*[S;I];
Id = eye(N+2);
Sdot = c*(Lambda*Id(:,1) - beta*SI_conv - mu*[S;0]);
Idot = c*(beta*SI_conv - (mu+gamma)*[I;0]);
TSdot = T*Sdot;
TIdot = T*Idot;
alt = 1-2*rem(1:N,2);
