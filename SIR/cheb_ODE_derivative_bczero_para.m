function Dpara = cheb_ODE_derivative_bczero_para(S,I,para,N)
...
Fp0 = [cheb_ODE(S(:,1),I(:,1),S(:,2),I(:,2),[1,0],para,0,N,0);
    zeros(2*(N+1),1);
    cheb_ODE(S(:,3),I(:,3),S(:,4),I(:,4),[1,0],para,2,N,0);
    zeros(2*(N+1),1);
    0;
    0];
Fp1 = [zeros(2*(N+1),1);
    zeros(2*(N+1),1);
    cheb_ODE(S(:,3),I(:,3),S(:,4),I(:,4),[0,1],para,2,N,0);
    zeros(2*(N+1),1);
    0;
    0];
Dpara = [Fp0,Fp1];
